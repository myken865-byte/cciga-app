...
export async function POST(
  ...
  if (submitted.length === 0) {
    return NextResponse.json({ error: "Aucune note soumise à mettre en vérification pour ce cours." }, { status: 400 });
  }

  await prisma.grade.updateMany({
    where: { courseId: id, status: "soumis" },
    data: { status: "en_verification" },
  });
  ...
  await writeAuditLog({
    ...
    reason: `${submitted.length} note(s) mise(s) en vérification pour ${course.name}.`,
  });
