...
const styles = StyleSheet.create({
  ...
  statusValue: { fontSize: 10, fontWeight: 700, color: "#00185a", marginTop: 6, letterSpacing: 1 },
...
export default function ReceiptDocument({
  ...
  /** "PAYÉ" ou "PARTIELLEMENT PAYÉ" — dérivé de totalPayé vs frais configurés, jamais inventé. */
  statusLabel: string;
  /** Mode de paiement réel ("Manuel", "MonCash", "NatCash"). */
  providerLabel: string;
  /** Référence de transaction du fournisseur — absente pour un paiement manuel. */
    ...
    <Document>
      <Page size="A4" style={shared.page}>
        ...
        <View style={shared.identityBlock}>
          ...
          <View style={shared.identityRow}>
            <Text style={shared.label}>Mode de paiement</Text>
            <Text style={shared.value}>{providerLabel}</Text>
          </View>
          {providerReference && (
            <View style={shared.identityRow}>
              <Text style={shared.label}>Référence de transaction</Text>
              <Text style={shared.value}>{providerReference}</Text>
            </View>
          )}
          {recordedByLabel && (
            <View style={shared.identityRow}>
              <Text style={shared.label}>Enregistré par</Text>
              <Text style={shared.value}>{recordedByLabel}</Text>
            </View>
          )}
          {note && (
            <View style={shared.identityRow}>
